...
library(rpart)
library(rpart.plot)
outcomecart <- rpart(Outcome~.,data=train_data, method="class")
prp(outcomecart)

pred = predict(outcomecart, newdata = test_data)
pred
pred.prob <- pred[,2]

table(test_data$Outcome, pred.prob >= 0.5)
(135+45)/(135+45+12+34)
...
#BaseLine model is 65% accurate but CART MODEL IS 80% accurate


#Model 2 
#Build Logistic Regression
result1 <- glm(data = train_data, Outcome ~ Glucose + BMI + DiabetesPedigreeFunction, family = binomial)
summary(result1)

predictTrain <- predict(result1, type = "response")
predictTrain
summary(predictTrain)

tapply(predictTrain, train_data$Outcome, mean)
table(train_data$Outcome, predictTrain > 0.5)
(304+104)/(304+104+80+40)
#77.77 accurate
